...
The OpenAPI (\Cref{sec:openapi}) specification is generated using Springdoc from the Spring framework (\Cref{sec:spring}), so the users can have general and straightforward API documentation available to them.
...
The Swagger UI (\Cref{sec:swagger}) is then used for user-friendly visualization of the documentation mentioned above.
